...
class GuessTheNumber:
  ...
  def __isWinner(self,num):
    return num == self.__numberToGuess

  def __getLeftAttempts(self,currentAttempt):
  ...
  def startGame(self):
    for i in range(self.__attempts):
      while True:
        try:  
          numero=int(input(f"Adivine el número de 1 a { self.__topNumber }, tiene { self.__getLeftAttempts(i) } intentos: "))
